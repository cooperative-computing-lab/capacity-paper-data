...
int capacity_test(int tasks, int sleep_time, char *name) {
	...
	work_queue_specify_name(wq, "wq_capacity");
	...
	int result = 0;
	int complete = 0;
	int timeout = (sleep_time * tasks) + 10;
	...
	while(time_acc < timeout) {
		...
		while(i < tasks) {
			...
			char *cmd = string_format("dd if=in.dat of=%s bs=4096 count=2500 ; sleep %d", out, sleep_time);
			struct work_queue_task *t = work_queue_task_create(cmd);

			work_queue_task_specify_file(t, "in.dat", "in.dat", WORK_QUEUE_INPUT, WORK_QUEUE_NOCACHE);
